...
const tally_use_https = process.env.TALLY_USE_HTTPS === '1' ||
...
function extractReport(reportConfig, reportInputParams) {
    return new Promise(async (resolve, reject) => {
        ...
        try {
            ...
            const findDataNode = (obj) => {
                if (obj?.['DATA'] !== undefined)
                    return obj['DATA'];
                if (obj?.['ENVELOPE']?.['DATA'] !== undefined)
                    return obj['ENVELOPE']['DATA'];
                if (obj?.['ENVELOPE']?.['BODY']?.['DATA'] !== undefined)
                    return obj['ENVELOPE']['BODY']['DATA'];
            ...
            const deepFindRowsByField = (obj, fieldId, depth = 0) => {
            ...
            if (reportConfig.output.datatype == 'array' && reportConfig.output.fields) {
                ...
                let data = processRows(rowArray, reportConfig.output.fields);
                retval.data = data;
            }
            else {
                if (dataNode && dataNode['ROW'] && !dataNode['ROW']['VALUE']) {
                    let _value = dataNode['ROW'][0]['VALUE'];
